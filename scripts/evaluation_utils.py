        ...
        for metrics in metrics_list:
            ...
            if not detected_task_type:
                if 'exact_match' in metrics or 'f1' in metrics:
                    detected_task_type = 'short_answer'
                elif any(key.startswith('rouge') for key in metrics.keys()):
                    detected_task_type = 'summarization'
                elif 'accuracy' in metrics and len([k for k in metrics.keys() if k.startswith('a@')]) == 0:
                    detected_task_type = 'multiple_choice'
                elif any(key.startswith('a@') for key in metrics.keys()):
                    detected_task_type = 'multiple_select'
                elif 'accuracy' in metrics:
                    detected_task_type = 'true_false'
            
            if detected_task_type:
                for metric_name, value in metrics.items():
                    if (metric_name in evaluation_results[detected_task_type] and 
                        not metric_name.endswith('_idx')):
                        evaluation_results[detected_task_type][metric_name].append(value)
                
                for metric_name, value in metrics.items():
                    if (metric_name in category_evaluation_results[category][detected_task_type] and 
                        not metric_name.endswith('_idx')):
                        category_evaluation_results[category][detected_task_type][metric_name].append(value)
